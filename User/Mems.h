...
enum class MemsStates : uint32_t {
    Running = 0,
    Pause,
    Initialize
};

enum class MemsEvents : uint32_t {
    None = 0,
    TriggerPause,
    TriggerMemsInitialization,
    TriggerResume,
    Sync
};

class Mems : public FsmBase<MemsStates, MemsEvents> {
public:
    using States = MemsStates;
    using Events = MemsEvents;

    static Mems& getInstance() {
        static Mems instance;
        return instance;
    }
...
private:
    Mems() : FsmBase<MemsStates, MemsEvents>() {
    ...
    void Initialize_TriggerMemsInitialization_callback(Msg msg);
...
std::vector<Transition> createTransitionTable() override {
    return {
        {{States::Initialize, Events::TriggerMemsInitialization}, States::Running, [this](Msg msg){this->Initialize_TriggerMemsInitialization_callback(msg);}},
...
std::string eventToString(Events event) const override {
    switch (event) {
        ...
        case Events::TriggerMemsInitialization: return "TriggerMemsInitialization";
